...
RSpec.describe RunApi::Suno::Resources::ConvertAudio do
  let(:http) { instance_double(RunApi::Core::HttpClient) }
  let(:resource) { described_class.new(http) }
  let(:endpoint) { "/api/v1/suno/convert_audio" }
  ...
  describe "#create" do
    it "POSTs to the correct endpoint" do
      ...
      expect(result).to be_a(RunApi::Suno::Types::ConvertAudioResponse)
  ...
  describe "#run" do
    it "creates then polls until complete" do
      ...
      expect(result).to be_a(RunApi::Suno::Types::CompletedConvertAudioResponse)
    end
  end
end
