...
RSpec.describe RunApi::Suno::Resources::GenerateMidi do
  let(:http) { instance_double(RunApi::Core::HttpClient) }
  let(:resource) { described_class.new(http) }
  let(:endpoint) { "/api/v1/suno/generate_midi" }
  let(:valid_params) { {task_id: "t1"} }
  ...
  describe "#create" do
    it "POSTs to the correct endpoint" do
      ...
      expect(result).to be_a(RunApi::Suno::Types::GenerateMidiResponse)
  ...
  describe "#run" do
    it "creates then polls until complete" do
      ...
      expect(http).to receive(:request).with(:get, "#{endpoint}/task-1")
        .and_return("id" => "task-1", "status" => "processing")
      expect(http).to receive(:request).with(:get, "#{endpoint}/task-1")
        .and_return("id" => "task-1", "status" => "completed", "instruments" => [{"name" => "Piano", "notes" => [{"pitch" => 60, "start_time" => 0.0, "end_time" => 1.0, "velocity" => 0.8}]}])

      allow(RunApi::Core::Polling).to receive(:sleep)
      result = resource.run(**valid_params)
      expect(result).to be_a(RunApi::Suno::Types::CompletedGenerateMidiResponse)
    end
  end
end
