...
RSpec.describe RunApi::Suno::Resources::GenerateArtwork do
  let(:http) { instance_double(RunApi::Core::HttpClient) }
  let(:resource) { described_class.new(http) }
  let(:endpoint) { "/api/v1/suno/generate_artwork" }
  ...
  describe "#create" do
    it "POSTs to the correct endpoint" do
      ...
      expect(result).to be_a(RunApi::Suno::Types::GenerateArtworkResponse)
  ...
  describe "#run" do
    it "creates then polls until complete" do
      ...
      expect(http).to receive(:request).with(:get, "#{endpoint}/task-1")
        .and_return("id" => "task-1", "status" => "processing")
      expect(http).to receive(:request).with(:get, "#{endpoint}/task-1")
        .and_return("id" => "task-1", "status" => "completed", "covers" => [{"url" => "https://cdn.runapi.ai/public/samples/cover.png"}])

      allow(RunApi::Core::Polling).to receive(:sleep)
      result = resource.run(**valid_params)
      expect(result).to be_a(RunApi::Suno::Types::CompletedGenerateArtworkResponse)
    end
  end
end
