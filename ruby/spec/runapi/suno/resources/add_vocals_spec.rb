...
RSpec.describe RunApi::Suno::Resources::AddVocals do
  let(:http) { instance_double(RunApi::Core::HttpClient) }
  let(:resource) { described_class.new(http) }
  let(:endpoint) { "/api/v1/suno/add_vocals" }
  let(:valid_params) { {upload_url: "https://cdn.runapi.ai/public/samples/audio-5.mp3", lyrics: "lyrics", title: "Song", negative_tags: "none", style: "pop", model: "suno-v5"} }
  ...
  describe "#run" do
    it "creates then polls until complete" do
      ...
      expect(http).to receive(:request).with(:get, "#{endpoint}/task-1")
        .and_return("id" => "task-1", "status" => "processing")
      expect(http).to receive(:request).with(:get, "#{endpoint}/task-1")
        .and_return("id" => "task-1", "status" => "completed", "audios" => [{"id" => "a5", "audio_url" => "https://cdn.runapi.ai/public/samples/audio-9.mp3"}])

      allow(RunApi::Core::Polling).to receive(:sleep)
      result = resource.run(**valid_params)
      expect(result).to be_a(RunApi::Suno::Types::CompletedAddVocalsResponse)
    end
  end
end
